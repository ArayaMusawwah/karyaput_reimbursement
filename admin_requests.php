...
if (isset($_POST['action']) && isset($_POST['request_id'])) {
    ...
    if ($action === 'approve' || $action === 'deny') {
        ...
        try {
            ...
            $success = Language::t("Request ") . $status . Language::t(" successfully!");
        } catch(PDOException $e) {
            $error = Language::t("Error updating request: ") . $e->getMessage();
        }
    } elseif ($action === 'pay') {
        ...
        try {
            ...
            $success = Language::t("Request marked as paid!");
        } catch(PDOException $e) {
            $error = Language::t("Error marking as paid: ") . $e->getMessage();
        }
    }
}
    ...
    <div id="toast-container" class="toast-container"></div>
        ...
        <!-- Search and Filter Section -->
                    ...
                    <div class="card-body">
                        <form method="GET">
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <input type="date" class="form-control" name="date_from" placeholder="<?php echo Language::t('From Date'); ?>" value="<?php echo isset($_GET['date_from']) ? htmlspecialchars($_GET['date_from']) : ''; ?>">
                                </div>
                                <div class="col-md-2">
                                    <input type="date" class="form-control" name="date_to" placeholder="<?php echo Language::t('To Date'); ?>" value="<?php echo isset($_GET['date_to']) ? htmlspecialchars($_GET['date_to']) : ''; ?>">
                                </div>
                                <div class="col-md-1">
                            ...
                            <div class="row mt-2">
                                <div class="col-md-12">
                                    <a href="admin_requests.php" class="btn btn-secondary"><?php echo Language::t('Clear Filter'); ?></a>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th><?php echo Language::t('ID'); ?></th>
                        <th><?php echo Language::t('Submitted By'); ?></th>
                        <th><?php echo Language::t('Description'); ?></th>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="btn-group-vertical" role="group">
                                <!-- Detail button -->
                                <a href="admin_request_detail.php?id=<?php echo $request['id']; ?>" class="btn btn-sm btn-info btn-block mb-1">
                                ...
                                <?php if ($request['receipt_path']): ?>
                                    ...
                                    ?>
                                    <?php if ($isImage): ?>
                                        <a href="<?php echo htmlspecialchars($request['receipt_path']); ?>" class="btn btn-sm btn-outline-primary btn-block mb-1" data-bs-toggle="modal" data-bs-target="#receiptModal<?php echo $request['id']; ?>">
                                            <?php echo Language::t('Receipt'); ?>
                                        </a>
                                    ...
                                    <?php else: ?>
                                        <a href="<?php echo htmlspecialchars($request['receipt_path']); ?>" target="_blank" class="btn btn-sm btn-outline-primary btn-block mb-1">
                                            <?php echo Language::t('Receipt'); ?>
                                        </a>
                                    <?php endif; ?>
                                <?php endif; ?>
                                ...
                                <?php if ($request['status'] === 'pending'): ?>
                                    <form method="POST" class="d-inline mb-1">
                                        ...
                                        <button type="button" class="btn btn-sm btn-success approve-btn btn-block"><?php echo Language::t('Approve'); ?></button>
                                    </form>
                                    
                                    <!-- Deny button with modal -->
                                    <button type="button" class="btn btn-sm btn-danger btn-block mb-1" data-bs-toggle="modal" data-bs-target="#denyModal<?php echo $request['id']; ?>">
                                    ...
                                    <div class="modal fade" id="denyModal<?php echo $request['id']; ?>" tabindex="-1">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"><?php echo Language::t('Deny Request'); ?> #<?php echo $request['id']; ?></h5>
                                                ...
                                                <form method="POST">
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="notes<?php echo $request['id']; ?>" class="form-label"><?php echo Language::t('Notes (optional)'); ?></label>
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <button type="submit" class="btn btn-danger"><?php echo Language::t('Deny Request'); ?></button>
                                                    </div>
                                                </form>
                                            </div>
                                        </div>
                                ...
                                <?php elseif ($request['status'] === 'approved'): ?>
                                    <form method="POST" class="d-inline">
                                        ...
                                        <input type="hidden" name="confirm_action" value="1">
                                        <button type="button" class="btn btn-sm btn-info pay-btn btn-block"><?php echo Language::t('Mark as Paid'); ?></button>
                                    </form>
                                <?php endif; ?>
                            </div>
                        </td>
    ...
    <script>
        ...
        document.querySelectorAll('.approve-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                showConfirmationToast(
                    '<?php echo addslashes(Language::t("Are you sure you want to approve this request?")); ?>',
                    function() {
                        // Submit the form
        ...
        document.querySelectorAll('.pay-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Show confirmation toast
                showConfirmationToast(
                    '<?php echo addslashes(Language::t("Are you sure you want to mark this as paid?")); ?>',
                    function() {
                        // Submit the form
                        const actionInput = document.createElement('input');
                        actionInput.type = 'hidden';
        ...
        function showConfirmationToast(message, callback) {
            ...
            const toastHTML = `
                <div id="${toastId}" class="toast toast-custom text-bg-warning" role="alert" aria-live="assertive" aria-atomic="true">
                    <div class="toast-header">
                        <strong class="me-auto"><?php echo Language::t('Confirmation'); ?></strong>
                        <button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>
                    </div>
                    <div class="toast-body">
                        <div>${message}</div>
                        <div class="mt-2">
                            <button class="btn btn-sm btn-success me-2 confirm-yes"><?php echo Language::t('Yes'); ?></button>
                            <button class="btn btn-sm btn-secondary confirm-no"><?php echo Language::t('No'); ?></button>
                        </div>
                    </div>
                </div>
            `;
