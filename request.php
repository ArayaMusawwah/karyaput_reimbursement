...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($validItems)) {
        $error = Language::t("Please add at least one item with valid name and amount.");
    }
    ...
    if (isset($_FILES['receipt']) && $_FILES['receipt']['error'] == 0) {
        ...
        if (in_array($filetype, $allowed)) {
            ...
            if (move_uploaded_file($_FILES['receipt']['tmp_name'], $upload_path)) {
                ...
                $error = Language::t("There was an error uploading your receipt.");
                Logger::error("Error uploading receipt", ['user_id' => $_SESSION['user_id'], 'error' => $error]);
            }
        } else {
            $error = Language::t("Invalid file type. Only JPG, JPEG, PNG, and PDF files are allowed.");
    ...
    if (empty($error)) {
        ...
        try {
            ...
            $success = Language::t("Reimbursement request submitted successfully!");
            ...
            $conn->rollback();
            $error = Language::t("Error submitting request: ") . $e->getMessage();
...
<head>
    ...
    <title><?php echo Language::t('Submit Reimbursement Request'); ?></title>
...
<body>
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-10">
                <h2><?php echo Language::t('Submit Reimbursement Request'); ?></h2>
                ...
                <form method="POST" enctype="multipart/form-data" id="requestForm">
                    <div class="mb-3">
                        <label for="description" class="form-label"><?php echo Language::t('Description'); ?></label>
                        <textarea class="form-control" id="description" name="description" rows="3" placeholder="<?php echo Language::t('Enter brief description of your reimbursement request'); ?>"><?php echo isset($_POST['description']) ? htmlspecialchars($_POST['description']) : ''; ?></textarea>
                    </div>
                    
                    <div class="row mb-3">
                        <div class="col-md-6">
                            <label for="category_id" class="form-label"><?php echo Language::t('Category'); ?></label>
                            <select class="form-control" id="category_id" name="category_id" required>
                                <?php foreach($categories as $category): ?>
                                    <option value="<?php echo $category['id']; ?>" <?php 
                                        // Auto-select category with ID 1 if none is selected via POST
                                        if ((!isset($_POST['category_id']) && $category['id'] == 1) || 
                        ...
                        <div class="col-md-6">
                            <label for="requested_date" class="form-label"><?php echo Language::t('Date of Expense'); ?></label>
                            <input type="date" class="form-control" id="requested_date" name="requested_date" required value="<?php echo isset($_POST['requested_date']) ? htmlspecialchars($_POST['requested_date']) : date('Y-m-d'); ?>">
                        </div>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label"><?php echo Language::t('Items'); ?></label>
                        <div id="items-container">
                            ...
                            if (!empty($postedItems) && is_array($postedItems)) {
                                foreach ($postedItems as $i => $it) {
                                    ...
                                    echo '<label class="form-label" for="item-name-' . $i . '">' . Language::t('Name') . ' *</label>';
                                    echo '<input id="item-name-' . $i . '" type="text" class="form-control item-name" name="items[' . $i . '][name]" required value="' . $nameVal . '" placeholder="' . Language::t('Enter item name') . '">';
                                    echo '</div>';
                                    echo '<div class="col-md-5">';
                                    echo '<label class="form-label" for="item-amount-' . $i . '">' . Language::t('Amount (Rp)') . ' *</label>';
                                ...
                                echo '<div class="row">';
                                echo '<div class="col-md-6">';
                                echo '<label class="form-label" for="item-name-0">' . Language::t('Name') . ' *</label>';
                                echo '<input id="item-name-0" type="text" class="form-control item-name" name="items[0][name]" required value="" placeholder="' . Language::t('Enter item name') . '">';
                                echo '</div>';
                                echo '<div class="col-md-5">';
                                echo '<label class="form-label" for="item-amount-0">' . Language::t('Amount (Rp)') . ' *</label>';
                        ...
                        <button type="button" class="btn btn-outline-primary" id="add-item">
                            + <?php echo Language::t('Add Item'); ?>
                        </button>
                        <span id="js-status" class="badge bg-info text-dark ms-2">JS: waiting</span>
                    </div>
                    
                    <div class="mb-3">
                        <label for="receipt" class="form-label"><?php echo Language::t('Receipt (optional)'); ?></label>
                        <input type="file" class="form-control" id="receipt" name="receipt" accept=".jpg,.jpeg,.png,.pdf" placeholder="<?php echo Language::t('Select receipt file...'); ?>">
                        <div class="form-text"><?php echo Language::t('Upload receipt image or PDF (JPG, PNG, PDF files allowed)'); ?></div>
                    </div>
                    
                    <div class="d-flex justify-content-between">
                        <div>
                            <strong><?php echo Language::t('Total Amount'); ?>: </strong>
                            <span id="total-amount">Rp0</span>
                        </div>
                        <div>
                            <button type="submit" class="btn btn-success"><?php echo Language::t('Submit Request'); ?></button>
                            <a href="dashboard.php" class="btn btn-secondary"><?php echo Language::t('Cancel'); ?></a>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        (function() {
            ...
            const langName = <?php echo json_encode(Language::t('Name')); ?>;
            const langAmount = <?php echo json_encode(Language::t('Amount (Rp)')); ?>;
            ...
            itemDiv.innerHTML = `
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input id="item-name-` + itemIndex + `" type="text" class="form-control item-name" name="items[` + itemIndex + `][name]" required placeholder="` + <?php echo json_encode(Language::t('Enter item name')); ?> + `">
                    </div>
                    <div class="col-md-5">
